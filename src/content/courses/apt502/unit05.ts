...
export const unit05: Unit = {
  ...
  components: [
    ...
    // 3 ── core instruction (five concept blocks) ────────────────────────────
    {
      ...
      body:
        'Marta is lowering herself off a thirty centimetre box on one leg, slowly, the way you asked. At the bottom she looks up: “What am I meant to be feeling here?” You start to say quadriceps and stop, because the honest answer is better than the poster behind her. Going up, her quadriceps produced a turning effort that straightens the knee, which anatomists call a knee extension **moment**. Coming down, they produce that same moment while the knee travels the other way. Nothing in the tissue changed. The task did. The poster says “step-up: glutes, quads” and has no column for direction of travel, load position, speed, or which end of the limb is fixed, which are the things that decide what a muscle is doing.\n\n' +
...

    {
      ...
      body:
        ...
        'Second, where the load enters and how it spreads. In a closed chain a single external force line, the ground reaction under the fixed foot, serves every joint at once, and its perpendicular distance to each joint axis sets that joint’s share of the demand. Move that line three centimetres and every joint changes demand in the same instant, some up and some down. In an open chain the resistance is applied at one point on one segment, so the whole external moment lands on one joint and cannot be shuffled anywhere else. That single fact explains why leg extensions produce quadriceps demand your client cannot dodge, and why squats produce quadriceps demand she can.\n\n' +
    ...
    },
    {
      ...
      body:
        ...
        'The last argument of this course is an anatomical one. The angle between the neck and the shaft of the femur varies across a wide normal adult range. **Femoral anteversion**, the forward twist of that neck relative to the shaft, varies by tens of degrees between healthy people and takes the whole arc of comfortable hip rotation with it. Hip socket depth and orientation vary, so does the twist along the tibia, and so do femur to tibia to torso proportions, which decide by geometry alone how far a trunk must travel forward to keep a bar over the mid-foot. Soft tissue varies as much. Palmaris longus is missing in a substantial minority of people and at very different rates between populations, plantaris in perhaps one in ten, extra heads turn up on biceps brachii, and the sciatic nerve has several documented relationships to piriformis. Attachment points differ between individuals by centimetres, which changes moment arms measurably: differences in the Achilles tendon’s moment arm are large enough to show up in running economy between runners. Fibre-type proportion in vastus lateralis ranges from roughly a sixth to more than four-fifths type I across healthy people.\n\n' +
    ...
    // 10 ── active-recall set ────────────────────────────────────────────────
    {
      ...
      cards: [
        ...
        { id: 'apt502-u5-r8', front: 'Name the three things that genuinely differ between open and closed chain.',
          back: 'Degrees of freedom: a fixed distal segment couples joint angles and creates compensation options. Load path: one ground reaction line serves every joint, versus one resistance point landing entirely on one joint. Biarticular behaviour: two-joint muscles go near-isometric and stop obeying single-joint intuition.' },
      ...
      questions: [
        ...
        { id: 'apt502-u5-q5', kind: 'multi', cognitive: 'analysis',
          ...
          options: [
            ...
            { id: 'b', text: 'Whether one external force line serves several joints or one resistance point loads a single joint', correct: true },
    ...
    // 12 ── teach-back (the signature oral practical) ────────────────────────
    {
      ...
      prompt:
        'This is the unit’s signature assessment and it is spoken, not written. An examiner names one movement and you have four minutes to map it aloud, with no notes: the joint motions and the positions they pass through, where the external load enters and what moment it creates at each joint, which muscles are placed to meet those moments and in which role, which tissues take the residual load, and what you would expect to limit the task first. Your movement: a forty-metre suitcase carry with a 24 kg kettlebell in the right hand only. Speak it through as you would to an examiner: precise, anatomical, and willing to say where you are uncertain.',
      modelAnswer:
        'The task is upright walking with a large one-sided load hanging from one hand, so almost nothing in it is a prime-mover problem. Start with the load path. The kettlebell hangs at arm’s length on the right, its line of gravity roughly through the right hand and well out to the side of the lumbar spine, so it applies a continuous right side-bending moment to the trunk for the whole forty metres. The muscles meeting that moment are on the left: left quadratus lumborum, left external and internal oblique, and left erector spinae, all working isometrically. Those are the same obliques an exercise chart files under rotation. Rectus abdominis is not flexing anything. It and transversus abdominis are contributing stiffness rather than motion, and transversus in particular cannot produce side-bending or turning at all, because its fibres run straight across.\n\n' +
        'At the pelvis and hip the demand alternates with every step, and this is the part people miss. In single-limb support the abductors of the stance hip, meaning gluteus medius and minimus with tensor fasciae latae, resist the pelvis dropping on the swing side. Because the load sits over the right, when the right foot is down the body’s weight line is pulled toward that hip and the right abductor demand falls relative to unloaded walking. When the left foot is down, the same load is displaced away from the stance hip and the left abductor demand rises sharply. So over forty metres the two hips are doing quite different amounts of work, while the trunk side-benders on the left work throughout with no rest phase.\n\n' +
        'At the shoulder girdle the load pulls the right scapula down and into some downward rotation, so upper trapezius and levator scapulae resist that, with rhomboids and middle trapezius holding the scapula against being pulled forward. The shoulder joint itself is essentially being pulled apart, and it is held by the rotator cuff, the superior capsule and the coracohumeral ligament rather than by any prime mover. Down the arm the demand is on grip: the long finger flexors working isometrically through the flexor retinaculum, and this is usually what fails first. Distally, the foot and ankle manage the side-to-side balance, with fibularis longus and brevis against rolling out and tibialis posterior on the other side, and the intrinsic foot muscles working the whole time.\n\n' +
        'Loading demands and limiters. Grip endurance almost always ends the set before anything else, which means the trunk stimulus is capped by the forearms. If the trunk is the target, straps or a heavier implement change what the exercise trains. The spine is under compression from the load plus the co-contraction holding the trunk upright, so this is not a low-compression exercise even though nothing is moving. And there is a breathing competition worth naming: the abdominal wall is doing stiffness and forced breathing out at the same time, so as the demand for air rises across the carry the postural contribution measurably degrades, which is why the trunk deteriorates at the far end rather than the near one.\n\n' +
        'Where I am uncertain: I cannot say how much of the trunk moment is met by active muscle rather than by passive fascia and pressure, and the relative contributions of the individual side-benders are not something I can measure on a gym floor. If I wanted the loaded side symmetrical, I would alternate hands by distance rather than by set, and I would expect the two sides to look different for reasons that come from her own anatomy as much as from her training history.',
      rubric: [
        ...
        { id: 'apt502-u5-t4', text: 'Identifies the tissues taking passive or residual load (shoulder capsule and cuff under distraction, spinal compression, foot structures) and names a plausible first limiter with its consequence for what the exercise trains' },
